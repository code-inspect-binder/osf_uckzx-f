...
resultsmotifs <- as.data.frame(rbind(results330,results350,results370,results390,results405,results415,results425,
                                     results435,results455,results470,results490,results600))
write.csv2(resultsmotifs, "P1analysis_newbins_results_bymotif.csv")
...
ggmean <- ggplot(resultsmotifs_summary,aes(x=DATE,y=MEAN)) + 
  ...
  x = "Year BCE", y = "mean H(A|M) across motifs") + 
...
ggmedian <- ggplot(resultsmotifs_summary,aes(x=DATE,y=MEDIAN)) + 
  ...
  x = "Year BCE", y = "median H(A|M) across motifs") +
  scale_x_reverse() + geom_line() + geom_point()
ggmedian

#Plot H(A|M) per motifs per date
rdates <- rev(dates)
plot(resultsmotifs$DATE, resultsmotifs$CEAuthoritiesMotifs, xlim = c(600,330), xaxt='n', xlab = "Year BCE", 
...
p1 <- ggplot(resultsmotifs, aes(x=DATE, y=CEAuthoritiesMotifs)) + geom_violin(trim=T) +
  ...
  labs(y= "H(A|M) per motifs", x = "Year BCE")
p1
...
ggmean <- ggplot(Nresultsmotifs_summary,aes(x=DATE,y=MEAN)) + 
  ...
  x = "Year BCE", y = "mean H(A|M)/H(A) across motifs") + 
...
ggmedian <- ggplot(Nresultsmotifs_summary,aes(x=DATE,y=MEDIAN)) + 
  ...
  x = "Year BCE", y = "median H(A|M)/H(A) across motifs") +
  scale_x_reverse() + geom_line() + geom_point()
ggmedian

#Plot H(A|M) per motifs per date
rdates <- rev(dates)
plot(resultsmotifs$DATE, resultsmotifs$NormCEAuthoritiesMotifs, xlim = c(600,330), xaxt='n', xlab = "Year BCE", 
     ylab = "H(A|M)/H(A) per motifs", main = "P1: normalized conditional entropy of authorities given motifs for each motif")
axis(1, at = rdates, labels = rdates)
...
p1 <- ggplot(resultsmotifs, aes(x=DATE, y=NormCEAuthoritiesMotifs)) + geom_violin(trim=T) +
  ...
  labs(y= "H(A|M)/H(A) per motifs", x = "Year BCE")
p1
...
#____________________________________________________________#
#REGRESSION ANALYSIS: GROUPING BY MOTIFS for normalized values
...
#____________________________________________________________#
#REGRESSION ANALYSIS: GROUPING BY MOTIFS for non-normalized values

resultsmotifs$MOTIFS <- rownames(resultsmotifs)
resultsmotifs$DATE <- as.numeric(as.character(resultsmotifs$DATE))
#dependent variable = non-normalized conditional entropy of authorities given motifs
#predictor = DATE; 13 time periods defined by the median year (e.g. 370 is a median year in a period 400-380 BCE)
