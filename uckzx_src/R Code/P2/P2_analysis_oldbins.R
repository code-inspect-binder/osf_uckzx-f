...
#Prediction 1: Motifs become more informative about a coin's denomination.
...
write.csv2(results, "P2analysis_oldbins_results.csv")
...
rdates <- rev(dates) #dates = centuries BCE
plot(results$DATE, results$CEDenominationsMotifs, xlim = c(6,4), xaxt='n', xlab = "Century BCE", 
     ylab = "H(D|d)", main = "P2: Conditional entropy of denominantions given designs")
...
plot(results$DATE, results$NormCEDenominationsMotifs, xlim = c(6,4), xaxt='n', xlab = "Century BCE", 
     ylab = "H(D|d) / H(D)", main = "P2: normalized conditional entropy of denominantions given designs")
axis(1, at = rdates, labels = rdates)


#____________________________________________________________#
#RE-ANALYSIS: GROUPING BY AUTHORITIES, by time period
...
resultspoleis <- as.data.frame(rbind(results4,results5,results6))
write.csv2(resultspoleis, "P2analysis_oldbins_results_bypolis.csv")

#____________________________________________________________#
#Plots with H(D|d) = non-normalized conditional entropy

#Getting statistics: mean, meadian, standard deviation among authorities per period for non-normalized conditional
...
ggmean <- ggplot(resultspoleis_summary,aes(x=DATE,y=MEAN)) + 
  ...
  x = "Century BCE", y = "mean H(D|d) across authorities") + 
...
ggmedian <- ggplot(resultspoleis_summary,aes(x=DATE,y=MEDIAN)) + 
  ...
  x = "Century BCE", y = "median H(D|d) across authorities") +
  scale_x_reverse() + geom_line() + geom_point()
ggmedian

#Plot H(D|d) per authority per date
plot(resultspoleis$DATE, resultspoleis$CEDenominationsMotifs, xlim = c(6,4), xaxt='n', xlab = "Century BCE", 
     ylab = "H(D|d) per authority", main = "P2: Conditional entropy of denominations given designs for each authority")
axis(1, at = c(6,5,4), labels = c("6","5","4"))
...
p1 <- ggplot(resultspoleis, aes(x=DATE, y=CEDenominationsMotifs)) + geom_violin(trim=T) +
  ...
  labs(y= "H(D|d) per authority", x = "Century BCE")
p1
...
#Getting statistics: mean, meadian, standard deviation among authorities per period for normalized conditional
#entropy of denomination given designs
...
ggmean <- ggplot(Nresultspoleis_summary,aes(x=DATE,y=MEAN)) + 
  labs(title =  "P2: Mean normalized conditional entropy of denominations given designs \nacross authorities", 
       x = "Century BCE", y = "mean H(D|d)/H(D) across authorities") + 
  scale_x_reverse() + geom_errorbar(aes(ymin=Nresultspoleis_summary$MEAN-Nresultspoleis_summary$SE, 
                                        ymax=Nresultspoleis_summary$MEAN+Nresultspoleis_summary$SE),width=.1) + 
  geom_line() + geom_point()
ggmean
...
ggmedian <- ggplot(Nresultspoleis_summary,aes(x=DATE,y=MEDIAN)) + 
  labs(title =  "P2: Median normalized conditional entropy of denominations given designs \nacross authorities", 
       x = "Century BCE", y = "median H(D|d)/H(D) across authorities") +
  scale_x_reverse() + geom_line() + geom_point()
ggmedian

#Plot H(D|d)/H(D) per authority per date
plot(resultspoleis$DATE, resultspoleis$NormCEDenominationsMotifs, xlim = c(6,4), xaxt='n', xlab = "Century BCE", 
     ylab = "H(D|d)/H(D) per authority", main = "P2: Normalized conditional entropy of denominations given \ndesigns for each authority")
axis(1, at = c(6,5,4), labels = c("6","5","4"))
...
p1 <- ggplot(resultspoleis, aes(x=DATE, y=NormCEDenominationsMotifs)) + geom_violin(trim=T) +
  scale_x_discrete(limits = rev(levels(resultspoleis$DATE))) +
  ggtitle("P2: Normalized conditional entropy of denominations given \ndesigns for each authority") +
  labs(y= "H(D|d)/H(D) per authority", x = "Century BCE")
p1
...
#predictor = DATE; 3 time periods (roughly 6th, 5th, 4th century BCE)
